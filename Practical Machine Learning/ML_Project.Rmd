...
For the resampling method, the k-cross validation method ("repeatedcv") is applied to reduce sampling bias.
...
The random forest model overall accuracy is `r conf_rf$overall[1]*100`%, significanlty greater than the linear discriminant analysis model.
...
## Train model: gradient boosted method (gbm)
...
The gradient boosted method (gbm) model overall accuracy is `r conf_gbm$overall[1]*100`%, significanlty greater than the linear discriminant analysis model and slightly better than the randon forest model. This is the best model and will be applied to predict the classe variable in the test dataset (test_clean).
